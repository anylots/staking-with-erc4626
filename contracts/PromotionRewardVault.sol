 ...
 */
contract PromotionRewardVault is IRewardVault, Ownable {
     ...
     *
     */
    function award(address recevier, uint256 amount) public override onlyOwner returns (bool)  {
        //给用户发放奖励
        IERC20(assetAddress).transfer(recevier, amount * (100-ratio) / 100);
        //发放佣金
        // IERC20(assetAddress).transfer(recevier, amount * ratio / 100);
        return true;
    }
     ...
     *
     */
    function getAssetAddress() public override view returns (address) {
        return assetAddress;
    }
}